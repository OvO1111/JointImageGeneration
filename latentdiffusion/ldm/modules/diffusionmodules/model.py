...
class Upsample(nn.Module):
    def __init__(self, in_channels, with_conv, dims=2):
        ...
        if self.with_conv:
            self.conv = self.conv_nd(in_channels,
                                        in_channels,
                                        kernel_size=3,
                                        stride=1,
                                        padding=1)
...
class Downsample(nn.Module):
    ...
    def forward(self, x):
        if self.with_conv:
            pad = (0,1,0,1) if self.dims == 2 else (0,1,0,1,0,1)
...
class ResnetBlock(nn.Module):
    def __init__(self, *, in_channels, out_channels=None, conv_shortcut=False,
                 dropout, temb_channels=512, dims=2):
        super().__init__()
...
def make_attn(in_channels, attn_type="vanilla", dims=3):
    ...
    if attn_type == "vanilla" and dims == 2:
        return AttnBlock2d(in_channels)
    if attn_type == "vanilla" and dims == 3:
...
class Encoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
                 attn_resolutions, dropout=0.0, resamp_with_conv=True, in_channels,
                 resolution, z_channels, double_z=True, use_linear_attn=False, attn_type="vanilla", dims=2,
                 **ignore_kwargs):
        super().__init__()
        if use_linear_attn: attn_type = "linear"
        self.ch = ch
...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
                 ...
                 attn_type="vanilla", dims=2, **ignorekwargs):
        super().__init__()
        if use_linear_attn: attn_type = "linear"
        self.ch = ch
        self.temb_ch = 0
        ...
        # z to block_in
        self.conv_nd = torch.nn.Conv2d if dims == 2 else torch.nn.Conv3d
        self.conv_in = self.conv_nd(z_channels,
                                       block_in,
                                       kernel_size=3,
                                       stride=1,
                                       padding=1)
...
class LatentRescaler(nn.Module):
    def __init__(self, factor, in_channels, mid_channels, out_channels, depth=2, dims=3):
        super().__init__()
        # residual block, interpolate, residual block
        self.factor = factor
        self.conv_nd = nn.Conv2d if dims == 2 else nn.Conv3d
        self.conv_in = self.conv_nd(in_channels,
                                 mid_channels,
                                 kernel_size=3,
                                 stride=1,
        ...
        if dims == 2:
            self.attn = AttnBlock2d(mid_channels)
        elif dims == 3:
            self.attn = AttnBlock3d(mid_channels)
        self.res_block2 = nn.ModuleList([ResnetBlock(in_channels=mid_channels,
                                                     out_channels=mid_channels,
                                                     temb_channels=0,
                                                     dropout=0.0, dims=dims) for _ in range(depth)])

        self.conv_out = self.conv_nd(mid_channels,
                                  out_channels,
                                  kernel_size=1,
                                  )
